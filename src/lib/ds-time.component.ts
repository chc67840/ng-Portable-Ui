 ...
 * Standardized API & styling hooks.
 */
@Component({
    selector: 'ds-time',
    standalone: true,
...
export class DsTimeComponent {
    ...
    @Input() step?: string; // seconds step (e.g., 1, 60)
    ...
    get computedLabelClass(): string { return this.labelClass || (this.theme?.controlLabel?.() ?? 'text-sm font-medium text-slate-700'); }
    ...
    applyCssVars() { if (!this.cssVars || !this.el?.nativeElement) return; const style = (this.el.nativeElement as HTMLElement).style; for (const [k, v] of Object.entries(this.cssVars)) { if (v == null) style.removeProperty(k); else style.setProperty(k, String(v)); } }
