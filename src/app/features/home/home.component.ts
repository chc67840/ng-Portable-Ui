...
export class HomeComponent {
    ...
    // Drawer & theme state (local for demo page)
    ...
    onTreeSelection(ev: any) { this._selectedTree.set(ev.selection); }
    ...
    onCountryChange(val: string | string[] | null) { this.selectedCountry = Array.isArray(val) ? (val[0] ?? null) : val; }
    onTagsChange(val: string | string[] | null) { this.selectedTags = Array.isArray(val) ? val : (val ? [val] : []); }
    onFrameworkChange(val: string | string[] | null) { this.selectedFramework = Array.isArray(val) ? (val[0] ?? null) : val; }
    onLazySelectChange(val: string | string[] | null) { this.selectedLazy = Array.isArray(val) ? (val[0] ?? null) : val; }
    onFeatureToggle(v: boolean) { this.featureEnabled = v; }
    onSelectLazyLoad(_e: Event) { console.log('Select lazy load triggered'); }
    onTagRemoved(e: Event) { const el = e.target as HTMLElement | null; if (el?.remove) el.remove(); }
    onBadgeRemoved(e: Event) { const el = e.target as HTMLElement | null; if (el?.remove) el.remove(); }
    openInfoPopover() { document.getElementById('infoPopover')?.setAttribute('open', ''); }
    closeInfoPopover() { document.getElementById('infoPopover')?.removeAttribute('open'); }
